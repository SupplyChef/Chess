...
@testset "Chess.jl" begin
    ...
    @testset "Evaluation components" begin
        ...
        # Plus mobility difference. We check it's at least 20.
        @test evaluate(b_a7).piece_activity - evaluate(b_a1).piece_activity >= 20

        # Knight outpost (+35)
        b_out = board_from_fen("4k3/8/8/4N3/8/8/8/4K3 w - - 0 1") # e5 (rank 4)
        b_no_out = board_from_fen("4k3/8/3p4/4N3/8/8/8/4K3 w - - 0 1") # e5 knight, d6 pawn
        # b_no_out: knight on e5 is NOT an outpost because black pawn on d6 can challenge it.
        # Delta = Outpost bonus (35)
