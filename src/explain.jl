...
function explain_move(result::SearchResult, b::Board, my_color::Color;
    ...
    genuinely_winning = swing >= 110 && !is_recap && result.score >= 50
    genuinely_losing  = swing <= -110 && !is_recap && result.score <= -60
...
function explain_pv_outcome(result::SearchResult, b::Board, my_color::Color)::String
    ...
    # • Winning capture: material swing ≥ 110cp means the opponent can't simply
    ...
    if !opp_in_check
        is_cap || return ""
        _pv_material_swing(result.pv, b) < 110 && return ""
    end
    ...
    mat_gain   = Δmat >= 110 ? _describe_material(Δmat,  queen_won,  rook_won)  : ""
    mat_loss   = Δmat <= -110 ? _describe_material(Δmat, queen_lost, rook_lost) : ""
